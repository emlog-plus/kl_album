...
foreach($navis as $navi){
	if($navi['url'] == '?plugin=kl_album-master'){
	...
	if($album === ''){
		if(empty($kl_album_info)){
			...
			while($row1 = $DB->fetch_array($query1)){
				$new_str_arr[$row1['album']] = time() - $row1['addtime'] < 3600*24*15 ? ' background:url(./content/plugins/kl_album-master/images/new.gif) no-repeat;' : '';
			}
				...
				if($val['restrict'] == 'private'){
					...
					if(isset($val['head']) && $val['head'] != 0){
						...
						if($DB->num_rows($iquery) > 0){
							...
							$photo_size = empty($val['w']) ? (function_exists('kl_album_change_image_size') ? kl_album_change_image_size($val['head'], EMLOG_ROOT.'/content/plugins/kl_album-master/'.$coverPath) : chImageSize(EMLOG_ROOT.'/content/plugins/kl_album-master/'.$coverPath, 100, 100)) : array('w'=>$val['w'], 'h'=>$val['h']);
						}else{
						...
						if($DB->num_rows($iquery) > 0){
							...
							$photo_size = empty($val['w']) ? (function_exists('kl_album_change_image_size') ? kl_album_change_image_size($irow['id'], EMLOG_ROOT.'/content/plugins/kl_album-master/'.$coverPath) : chImageSize(EMLOG_ROOT.'/content/plugins/kl_album-master/'.$coverPath, 100, 100)) : array('w'=>$val['w'], 'h'=>$val['h']);
						}else{
							$coverPath = 'images/no_cover_s.jpg';
							$photo_size = array('w'=>100, 'h'=>100);
						}
...
<tr><td style="border:0px solid #CECECE; height:125px; vertical-align:middle; text-align:center; padding:0px;'.$new_str_arr[$val['addtime']].'"><a href="./?plugin=kl_album-master&album='.$val['addtime'].'" title="'.$val['description'].'"><img style="border:0px; padding:5px 5px 5px;" src="./content/plugins/kl_album-master/'.$coverPath.'" width="'.$photo_size['w'].'" height="'.$photo_size['h'].'"></a></td></tr>
	...
	if($album !== ''){
		...
		if(is_array($kl_album_info)){
			...
			if($exist_album === false || ($albumrestrict == 'private' && ROLE != 'admin')){
				...
				$photos = array();
				$log_content .= '
<div style="text-align:left; font-size:12px; padding:0px 20px;"> <a href="./?plugin=kl_album-master">&laquo;返回相册列表</a></div>
...
<img style="border:0px; padding:5px 5px 5px;" src="'.substr($photo['filename'], 1, strlen($photo['filename'])).'" width="'.$photo_size['w'].'" height="'.$photo_size['h'].'" /></a>
</li>';
				}
...
function kl_album_show_js(){
	...
	echo '<script type="text/javascript" src="./content/plugins/kl_album-master/js/jquery.lazyload.mini.js"></script>
<script type="text/javascript" src="./content/plugins/kl_album-master/js/jquery.lightbox-0.5.js"></script>
<link rel="stylesheet" type="text/css" href="./content/plugins/kl_album-master/css/jquery.lightbox-0.5.css" media="screen" />
<script type="text/javascript">
jQuery(function($){
$(\'img\').lazyload({effect:\'fadeIn\',placeholder:\'./content/plugins/kl_album-master/images/grey.gif\',threshold:200});
